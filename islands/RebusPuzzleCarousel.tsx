...
export default function RebusPuzzleCarousel({
  ...
  const dynamicImages = useSignal<Record<Difficulty, string[]>>({
    demo: [],
    easy: [],
    medium: [],
    hard: [],
  ...
  const fetchImages = async (difficulty: Difficulty) => {
    ...
    try {
      const response = await fetch(`/api/rebus/${difficulty}`);
  ...
  return (
    ...
    >
      ...
      {/* Thumbnail Navigation - Quick puzzle selection */}
      {currentImages.value.length > 1 && (
        <div
